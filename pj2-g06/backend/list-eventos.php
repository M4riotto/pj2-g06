...
<?php
    ...
    try {
        $stmt = $connect->prepare("SELECT  id, nome, capa, descricao, dia, horario FROM eventos;");
        $stmt->execute();

        $eventos = $stmt->fetchAll(PDO::FETCH_ASSOC);

        $result["success"]["message"] = "Evento listados com sucesso"; //criamos o array para devolver o resultado do insert numa mensagem de sucesso.
